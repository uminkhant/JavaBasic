...
public class DashBoardController implements Initializable {
	...
	void addDisease() {
		...
		try {
			...
			d.setDoctor(d.getDoctor());
			d.setPatient(d.getPatient());
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		...
		tv_patient.setOnMouseClicked(e -> {
			if (e.getClickCount() == 2) {
				Disease p=tv_patient.getSelectionModel().getSelectedItem();
				lb_patient.setText(p.getPatient().getName());
			}
		});
	}
	...
	private void loadView(Doctor d) {
		...
		pi_patient.setProgress(0.5);
		tf_doctor.setText(d.getName());
	}
